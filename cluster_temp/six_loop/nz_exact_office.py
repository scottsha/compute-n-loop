...
class NeumannZagierDatum():
    ...
    def generate_nz_data(self):
        ...
        new_shapes = [K(str(s.lift()).replace('x', 'y'))\
            for s in shapes]
        pol = K.polynomial()
        embedding = self.compute_ptolemy_field_and_embedding()
...
class nloop():
    ...
    def __init__(self, nzdata, n, diagrams):
        ...
        (A, B, nu, f, f_ddot, p, zees, _) = nzdata
        self.A = A
        self.B = B
        self.nu = nu
        self.f = f
