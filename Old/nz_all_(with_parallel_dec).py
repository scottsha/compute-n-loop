...
class NeumannZagierDatum():
    ...
    def generate_nz_data(self, file_name='None'):
        ...
        new_nz = (A, B, nu, f, f_ddot, new_shapes)
        if file_name != 'None':
    ...
    def one_loop(self, precision=None, shapes=None):
        ...
        return (1/CC(2)) * det(A*D1 + B*D2) *\
            self.exponentiate_list(shapes, fdd) *\
            self.exponentiate_list(shapes_dd, -f)
...
class nloop():
    ...
    def __init__(self, nzdata, n, diagrams):
        ...
        (A, B, nu, f, f_ddot, zees) = nzdata
        self.A = A
        self.B = B
        self.nu = nu
        self.f = Matrix(f).transpose()
        self.f_ddot = f_ddot
        self.zees = zees
    ...
    def gamma(self, eye, kay, ell):
        ...
        if kay == 0:
            return sum([self.polylogarithm(2 - self.n, 1/z) for z \
                ...
                (self.f.transpose() * self.B.inverse() * self.A *\
                self.f/8)[0][0]
