...
def cusp_shape(M, precision=100):
    rho = hypertorsion.polished_holonomy(M, precision)
    z = rho.cusp_shape()
    return z.conjugate()

def stavros_torsion(M, N = 2, precision=100):
    p = hypertorsion.hyperbolic_SLN_torsion(M, N, precision)
    if N % 2 == 0:
        return p(1)
    else:
        q = p.derivative()
        return q(1)/cusp_shape(M, precision)
...
class NeumannZagierDatum():
    ...
    def exact_shapes(self, precision, degree):
        T = snap.tetrahedra_field_gens(self.manifold)
        K, place, shapes = T.find_field(precision, degree, True)
        shapes = [K(s) for s in shapes]
        rot_shapes = [ all_shape_parameters(z)[(i - 1) % 3]
                 for z, i in zip(shapes, self._eliminated_shapes)]
        return K, place, [K(s) for s in rot_shapes]

    def exact_shapes_for_mathematica(self, precision, degree):
        T = snap.tetrahedra_field_gens(self.manifold)
        K, place, shapes = T.find_field(precision, degree, True)
        shapes = [K(s) for s in shapes]
        rot_shapes = [ all_shape_parameters(z)[(i - 1) % 3]
                 for z, i in zip(shapes, self._eliminated_shapes)]
        answer = K.absolute_polynomial(), place(100), rot_shapes
        return mathematica(answer)
    ...
    def exact_shapes_via_ptolemy_lifted(self):
        ...
        sol = [ c for c in s if is_geom(self.manifold,c) ][int(0)]
        cr = sol.cross_ratios()
        shapes = [cr['z_0000_%d' % i] for i in range(M.num_tetrahedra())]
        rot_shapes = [ all_shape_parameters(z)[(i - 1) % 3] \
                 for z, i in zip(shapes, self._eliminated_shapes)]
    ...
    def check_one_loop(self, precision=100):
        ratio = stavros_torsion(self.manifold, self.N+1, precision)/self.one_loop(precision)
        error = min( abs(1 - ratio), abs(1 + ratio) )
        assert error < epsilon(ratio.parent(), 2)
