...
class NeumannZagierDatum():
    ...
    def compute_ptolemy_field_and_embedding_magma(M):
        vol = M.volume()
        p = M.ptolemy_variety(2,'all')
        for sol in p.compute_solutions(verbose=False,engine="magma")\
            .flatten(depth=2):
            rsol = zip(pari('polroots(%s)' %sol.number_field()),\
            sol.numerical())
        for root, numerical_sol in rsol:
            if abs(vol -numerical_sol.volume_numerical())<1e-10:
                return root, sol.number_field()


    def exact_shapes_via_ptolemy_lifted(self):
        p = self.manifold.ptolemy_variety(2, 'all')
        s = p.compute_solutions(self.engine, self.verbose).flatten(depth=2)
        sol = [c for c in s if self.is_geom(self.manifold, c)][int(0)]
        cr = sol.cross_ratios()
        shapes = [cr['z_0000_%d' % i] for i in range(self.num_shapes)]
        rot_shapes = [self.all_shape_parameters(z)[(i - 1) % 3] \
    ...
    def generate_nz_data(self):
        ...
        A = self.ABv_square()[0]
        B = self.ABv_square()[1]
        nu = self.ABv_square()[2]
        f = self.f_and_fddot()[0]
        f_ddot = self.f_and_fddot()[1]
        p, shapes = self.exact_shapes_via_ptolemy_lifted()
        S = PolynomialRing(QQ, 'x')
        K = NumberField(S(str(p)), 'y')
        new_shapes = [K(str(s.lift()).replace('x', 'y'))\
            for s in shapes]
        pol = K.polynomial()
...
class nloop():
    ...
    def one_loop(self):
        ...
        return (1/QQ(2)) * det(self.A*D1 + self.B*D2) * \
    ...
    def gamma(self, eye, kay, ell):
        ...
        return (-1) ** kay * sum([h ** (bar - 1) / factorial(bar) *
            ...
            self.n - ell)]) - kronecker_delta(kay, 1) * QQ(1)/2 * \
    ...
    @parallel
    def diagram_contribution_to_nloop(self, diagram, ver_factor):
        ...
        for foo in range(N ** diagram.num_verts()):
            ...
            temp_sum += prod([prop[indices[eee[0]]]
                ...
                (labels=False)] +
                [ver_factor[self.feynman_loop_number(diagram)]
                [diagram.degree()[vee]][indices[vee]] for vee in
                diagram.vertices()])
        ans = self.symmetry_factor(diagram) * temp_sum *\
            (h ** (-self.n + 1))
    ...
    def nloop_invariant(self):
        ...
        vf = self.vertex_factor_tensor()

        temp = list(self.diagram_contribution_to_nloop([(g, vf) for\
            g in self.diagrams]))
        collect_results = [None]*len(temp)
        for i in range(len(temp)):
            collect_results[i] = temp[i][1]
        loop_invar = sum(collect_results) + vf[self.n][0][0]
        return loop_invar


def nloop_from_manifold(manifold, n, diagrams, engine=None, verbose=False, file_name=None):
...
def nloop_from_nzdatum(nz, n, diagrams, engine=None, verbose=False, file_name=None):
